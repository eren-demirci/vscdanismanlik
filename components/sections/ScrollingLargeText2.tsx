...
const ScrollingLargeText2 = ({ data }: { data: SectionProps }) => {
    const {
        wrapperCls,
        imageList2,
    } = data || {};
    ...
    return (
        <div className={`running-content has-large-text has-bg mt-100 ${wrapperCls}`}>
            <div className="content-inner">
                <div className="content-lists running-animation">
                    {Array.from({ length: 4 }).map((_, i) => (
                        <div className="content-item" key={i}>
                            {imageList2 && imageList2.map((item, index) => (
...
export default ScrollingLargeText2;